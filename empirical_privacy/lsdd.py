...
def lsdd(X1,X2,T=None, folds=5, sigma_list=None, lda_list=None):
    ...
    for sigma_idx, sigma in enumerate(sigma_list):
        ...
        for k in cv_fold:
            h1_cv[:, k] = np.sum(np.exp(-X1C_dist2[:, cv_index1==k]/(2*sigma**2)), axis=1)
            h2_cv[:, k] = np.sum(np.exp(-X2C_dist2[:, cv_index2==k]/(2*sigma**2)), axis=1)

        for k in cv_fold:
            # calculate the h vectors for training and test
            htr = np.sum(h1_cv[:, cv_fold!=k], axis=1)/np.sum(n1_cv[cv_fold!=k]) \
                -np.sum(h2_cv[:, cv_fold!=k], axis=1)/np.sum(n2_cv[cv_fold!=k])
            hte = np.sum(h1_cv[:, cv_fold==k], axis=1)/np.sum(n1_cv[cv_fold==k]) \
                -np.sum(h2_cv[:, cv_fold==k], axis=1)/np.sum(n2_cv[cv_fold==k])
    ...
    H = (np.sqrt(np.pi)*sigma_chosen)**d*np.exp(-CC_dist2/(4*sigma_chosen**2))
    h = np.mean(np.exp(-X1C_dist2/(2*sigma_chosen**2)), axis=1) - np.mean(np.exp(-X2C_dist2/(2*sigma_chosen**2)), axis=1)
...
def CalcDistanceSquared(X, C):
    ...
    XC_dist2 = Xsum[np.newaxis, :] + Csum[:, np.newaxis] - 2*np.dot(C.transpose(), X)
